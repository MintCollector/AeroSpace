...
aerospace workspace [-h|--help] --view-toggle <workspace-name>
...
--view-toggle::
Temporarily merge the windows of `<workspace-name>` into the currently focused
workspace (inspired by AwesomeWM's toggle tag view). Running it again with the
same workspace dissolves the merge, as does any workspace switch. State is
ephemeral (in-memory only).
Incompatible with `--auto-back-and-forth`, `--wrap-around`, and `--fail-if-noop`
