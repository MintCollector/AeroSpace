...
* Toggle "sticky" mode of a floating window: `sticky` (a sticky floating window is shown on all workspaces of its monitor; not valid with `--root`)
...
+
Succeeds even when no window is focused (the command operates on the root container directly), so it works on empty or floating-only workspaces.
+
