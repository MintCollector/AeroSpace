...
For about a second after the window appears, AeroSpace refuses to follow the window's native
...
For rules whose `if` conditions only reference the app (`app-id`, `app-id-regex-substring`,
`app-name-regex-substring`), the protection starts immediately on window creation — before the
window is placed in the layout. Rules conditioned on window title or workspace can only take
effect once the window is detected.
