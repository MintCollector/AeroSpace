...
struct SetFrameCommand: Command {
    ...
    func run(_ env: CmdEnv, _ io: CmdIo) async -> BinaryExitCode {
        ...
        guard window.isFloating else {
            return .fail(io.err("set-frame only works on floating windows"))
        }

        guard let currentRect = try? await window.getAxRect(.cancellable) else {
            return .fail(io.err("Can't get window frame"))
        }
        ...
        if newWidth <= 0 || newHeight <= 0 {
            return .fail(io.err("Width and height must be positive"))
        }
