...
func parseLayoutCmdArgs(_ args: StrArrSlice) -> ParsedCmd<LayoutCmdArgs> {
    ...
    .filterNot("'tiling', 'floating', 'unmanaged' and 'sticky' are incompatible with --root") {
        $0.root && $0.toggleBetween.val.contains(where: { $0 == .tiling || $0 == .floating || $0 == .unmanaged || $0 == .sticky })
    }
}
